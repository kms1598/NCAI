Shader "Custom/ClayURP_Floor"
{
    Properties
    {
        ...
        _NormalStrength ("Normal Strength", Range(0.0, 2.0)) = 1.10
        ...
        _ColorLift ("Lit Color Lift", Range(0.0, 0.5)) = 0.08
        _ShadowLevel ("Shadow Level", Range(0.2, 0.9)) = 0.50

        _Roughness ("Roughness", Range(0.0, 1.0)) = 0.96
        _SpecularStrength ("Specular Strength", Range(0.0, 1.0)) = 0.015
        _HorizontalMatte ("Horizontal Matte", Range(0.0, 1.0)) = 0.92

        _RimPower ("Rim Power", Range(0.1, 8.0)) = 3.5
        _RimStrength ("Rim Strength", Range(0.0, 1.0)) = 0.05
        _WrapAmount ("Wrap Amount", Range(0.0, 1.0)) = 0.45
        _WrapContrast ("Wrap Contrast", Range(0.5, 4.0)) = 1.8
        _ShadowSoftness ("Shadow Softness", Range(0.0, 1.0)) = 0.30
        _BandingStrength ("Banding Strength", Range(0.0, 1.0)) = 0.10
        _BandSteps ("Band Steps", Range(2.0, 8.0)) = 4.0
        _SheenStrength ("Clay Sheen Strength", Range(0.0, 1.0)) = 0.02
        _SheenWidth ("Clay Sheen Width", Range(0.1, 1.0)) = 0.35

        _ClayVariationTex ("Clay Variation Noise", 2D) = "gray" {}
        _ClayVariationScale ("Variation Scale", Range(0.5, 20.0)) = 5.0
        _ClayVariationStrength ("Variation Strength", Range(0.0, 0.3)) = 0.04
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 BlendLight(float3 normalWS, float3 viewDirWS, float3 albedo, float3 lightDir, float3 lightColor, float atten)
            {
                ...
                float horizontalMask = abs(normalWS.y);
                ...
                // Floors: milder lift so dark wood/velvet stay deep, not milky.
                float3 litCol = ScaleLumaPreserveChroma(albedo, 1.15, _ColorLift);
                litCol *= lerp(float3(1.0, 1.0, 1.0), float3(1.02, 0.99, 0.97), warmth * 0.25);
                ...
                float specPow = lerp(16.0, 110.0, 1.0 - rough);
                float specIntensity = _SpecularStrength * pow(max(1.0 - rough, 0.0), 2.0) * glossScale;
                ...
                float sheenExp = lerp(14.0, 5.0, _SheenWidth);
                float3 sheen = pow(edgeMask, sheenExp) * stylizedDiffuse * _SheenStrength * glossScale * litCol * lightColor;
            ...
            float4 frag(Varyings IN) : SV_Target
            {
                ...
                float3 rimCol = ScaleLumaPreserveChroma(albedo, 1.25, 0.03) * _RimColor.rgb;
                float rim = pow(1.0 - saturate(dot(normalWS, viewDirWS)), _RimPower) * _RimStrength;
                // Extra rim kill on floors.
                float floorMask = abs(normalWS.y);
                rim *= 1.0 - floorMask * 0.85;
