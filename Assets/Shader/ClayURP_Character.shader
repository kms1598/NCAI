Shader "Custom/ClayURP_Character"
{
    Properties
    {
        ...
        _NormalStrength ("Normal Strength", Range(0.0, 2.0)) = 0.55
        ...
        _PaletteWarmth ("Palette Warmth", Range(0.0, 1.0)) = 0.18
        _ColorLift ("Lit Color Lift", Range(0.0, 0.5)) = 0.10
        _ShadowLevel ("Shadow Level", Range(0.2, 0.9)) = 0.58

        _Roughness ("Roughness", Range(0.0, 1.0)) = 0.90
        _SpecularStrength ("Specular Strength", Range(0.0, 1.0)) = 0.04

        _RimPower ("Rim Power", Range(0.1, 8.0)) = 2.8
        _RimStrength ("Rim Strength", Range(0.0, 1.0)) = 0.14
        _WrapAmount ("Wrap Amount", Range(0.0, 1.0)) = 0.50
        _WrapContrast ("Wrap Contrast", Range(0.5, 4.0)) = 1.6
        _ShadowSoftness ("Shadow Softness", Range(0.0, 1.0)) = 0.32
        _BandingStrength ("Banding Strength", Range(0.0, 1.0)) = 0.08
        _BandSteps ("Band Steps", Range(2.0, 8.0)) = 4.0
        _SheenStrength ("Clay Sheen Strength", Range(0.0, 1.0)) = 0.06
        _SheenWidth ("Clay Sheen Width", Range(0.1, 1.0)) = 0.45

        _ClayVariationTex ("Clay Variation Noise", 2D) = "gray" {}
        _ClayVariationScale ("Variation Scale", Range(0.5, 20.0)) = 5.0
        _ClayVariationStrength ("Variation Strength", Range(0.0, 0.3)) = 0.03

        [Header(Character Bloom)]
        _BloomColor ("Bloom Color", Color) = (1.0, 0.96, 0.90, 1.0)
        _BloomIntensity ("Bloom Intensity", Range(0.0, 3.0)) = 0.55
        _BloomThreshold ("Bloom Threshold", Range(0.0, 2.0)) = 0.65
        _BloomSoftness ("Bloom Softness", Range(0.05, 1.0)) = 0.35
        _BloomRimBoost ("Bloom Rim Boost", Range(0.0, 2.0)) = 0.85
        _BloomFaceBoost ("Bloom Lit Boost", Range(0.0, 2.0)) = 0.45
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                float _BloomRimBoost;
                float _BloomFaceBoost;
            CBUFFER_END
            ...
            // Brighten/darken while keeping the albedo hue.
            ...
            float3 SoftBloomGlow(float3 color, float3 albedo, float3 normalWS, float3 viewDirWS, float lightVisibility)
            {
                float luma = Luma(color);
                float soft = max(_BloomSoftness, 0.05);
                float brightMask = saturate((luma - _BloomThreshold) / soft);

                float fresnel = pow(1.0 - saturate(dot(normalWS, viewDirWS)), 2.2);
                float litMask = saturate(lightVisibility);
                float glowMask = saturate(brightMask * (0.35 + _BloomFaceBoost * litMask) + fresnel * _BloomRimBoost * litMask);

                // HDR-ish emission so URP Volume Bloom can catch it.
                float3 tint = lerp(albedo, _BloomColor.rgb, 0.65);
                return tint * glowMask * _BloomIntensity;
            }
            ...
            float4 frag(Varyings IN) : SV_Target
            {
                ...
                // Character soft bloom / emissive glow
                color += SoftBloomGlow(color, albedo, normalWS, viewDirWS, lightVisibility);
